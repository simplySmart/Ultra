...
import { Search, Bookmark, Menu, Download, Monitor, SlidersHorizontal, List, LayoutGrid, Loader2, ChevronLeft, ChevronRight, Calendar as CalendarIcon } from 'lucide-react';
...
export default function App() {
  ...
  if (selectedAnimeId) return <AnimeViewer animeId={selectedAnimeId} onBack={closeAnime} />;
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB] text-gray-900 font-sans pb-10">
      <nav className="bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between sticky top-0 z-50 shadow-sm">
        ...
        <div className="flex items-center gap-6 text-gray-600">
          ...
          <button 
            ...
            className={`transition-colors p-2 rounded-lg ${currentTab === 'home' ? 'bg-purple-100 text-purple-700' : 'hover:bg-gray-50 hover:text-purple-600'}`}
          >
            <Bookmark size={22} />
          </button>
          <button 
            onClick={() => { setCurrentTab('schedule'); window.scrollTo(0,0); }} 
            className={`transition-colors p-2 rounded-lg ${currentTab === 'schedule' ? 'bg-purple-100 text-purple-700' : 'hover:bg-gray-50 hover:text-purple-600'}`}
          >
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        {currentTab === 'schedule' ? (
          <Schedule onOpenAnime={openAnime} />
        ) : (
          <>
            <div className="mb-8">
            ...
            <div className="flex flex-wrap gap-4 mb-6">
              <div className="relative flex-1 min-w-[160px]">
                ...
                <select value={filters.resolution} onChange={(e) => { setFilters({ resolution: e.target.value }); setCurrentPage(1); }} className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-xl appearance-none outline-none focus:border-purple-300 focus:ring-2 focus:ring-purple-100 font-medium text-gray-700">
                  <option>All Res</option><option>1080p</option><option>720p</option>
                </select>
              </div>
              <button className="bg-purple-50 text-purple-600 p-2.5 rounded-xl border border-purple-100 hover:bg-purple-100 shrink-0"><SlidersHorizontal size={22} /></button>
            </div>
            
            <div className="flex items-center justify-end bg-white border border-gray-100 rounded-xl p-1 mb-8 shadow-sm">
              <div className="flex gap-1 px-2">
                <button onClick={() => setViewMode('list')} className={`p-2 rounded-lg transition-colors ${viewMode === 'list' ? 'bg-purple-100 text-purple-700' : 'text-gray-400 hover:text-gray-600'}`}><List size={20} /></button>
                <button onClick={() => setViewMode('grid')} className={`p-2 rounded-lg transition-colors ${viewMode === 'grid' ? 'bg-purple-100 text-purple-700' : 'text-gray-400 hover:text-gray-600'}`}><LayoutGrid size={20} /></button>
              </div>
            </div>

            {loading ? (
              <div className="flex flex-col items-center justify-center py-20 text-gray-400 gap-4"><Loader2 className="animate-spin text-purple-500" size={32} /><p>Syncing release feed...</p></div>
              ...
              <div className="flex flex-col gap-6">
                {Object.entries(groupedItems).map(([dateLabel, groupItems]) => (
                  <div key={dateLabel}>
                    ...
                    <div className={`grid gap-4 ${viewMode === 'grid' ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'}`}>
                      {groupItems.map(item => (
                        <ReleaseCard key={item.id} item={item} viewMode={viewMode} onClick={() => openAnime(item.anime_id || item.id.substring(0, item.id.lastIndexOf('-')))} />
                      ))}
                    </div>
                  </div>
                ))}
                {totalPages > 1 && (
                  <div className="flex justify-center items-center gap-6 mt-10">
                    <button onClick={() => { setCurrentPage(p => Math.max(1, p - 1)); window.scrollTo(0, 0); }} disabled={currentPage === 1} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white border border-gray-200 text-gray-700 disabled:opacity-40 font-bold hover:bg-purple-50 hover:text-purple-700 transition-colors shadow-sm"><ChevronLeft size={18} /> Prev</button>
                    <span className="text-gray-500 font-bold text-sm">Page {currentPage} of {totalPages}</span>
                    <button onClick={() => { setCurrentPage(p => Math.min(totalPages, p + 1)); window.scrollTo(0, 0); }} disabled={currentPage === totalPages} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white border border-gray-200 text-gray-700 disabled:opacity-40 font-bold hover:bg-purple-50 hover:text-purple-700 transition-colors shadow-sm">Next <ChevronRight size={18} /></button>
                  </div>
                )}
              </div>
            )}
